...
if __name__ == '__main__':
    ...
    retain_content = ['a waterbird', 'a landbird']
    interaction_pre = ['with']
    interaction_post = ['background']
    bias_content = ['land', 'wood', 'forest', 'mountain', 'water', 'ocean', 'beach']
    retain_prompt, bias_prompt, candidate_prompt, index_mapping = (
        create_prompts_and_index_mapping(templates, retain_content, bias_content, interaction_pre, interaction_post))
    ...
    for epoch in range(args.epochs):  # Number of epochs
        ...
        for i, cls_emb in enumerate(z_tar):
            for j, (f, s) in enumerate(combi(uq_pair, 2)):
                ...
                equalization_loss += F.mse_loss(cls_emb @ z_dif[bias_len + f].T, cls_emb @ z_dif[bias_len + s].T).to(torch_device)
    ...
    prompt = ['a photo of waterbird', 'a photo of waterbird with land background', 'a photo of landbird',
              'a photo of landbird with water background']
    ...
    # Diffusion Sampling
    save_dir = (f"241017_bird" + args.cls + "_lam" + str(args.lam)
