...
class KernelizedEncoder(nn.Module):
    ...
    def forward(self, x):
        x = x.to(dtype=self.dtype)
        # 768 -> 3000 ->
        phi_x = torch.sqrt(torch.tensor([2./len(self.w)], device=x.device)) * torch.cos(torch.mm(x, self.w.t().to(dtype=self.dtype))
                                                                                        + self.b.to(dtype=self.dtype))
        z = torch.mm(phi_x, self.U) # (batch,3000) @ (3000,1)
        return z.float()
