...
class PGD():
    def __init__(self, model=None, bound=None, step=None, iters=None, norm=False, random_start=False, discrete=True,
                 device=None, P=None, P_info=None, no_label=False, using=None, mode=None, **kwargs):
    ...
    def perturb_bafa(self, x, query_embeddings, debais_well_q, target_y=None, model=None, y_best_group=None, mode=None,
        ...
        for i in range(iters):  # TODO argmax need to debias Y. So before argmax please use P0 Sigma-1
            ...
            if self.no_label:
                ...
                # loss = F.cross_entropy(adv_ori_logit, target_y) - F.cross_entropy(adv_debais_logit, target_y)
                # loss = F.cross_entropy(chat_adv, target_y)
                ...
                log_sum_ori = torch.log(torch.sum(torch.exp(adv_ori), dim=1, keepdim=True)).detach()
                term2 = torch.abs(log_sum_de - log_sum_cha).mean() / log_sum_ori.mean()

                # mean_vector = adv_debais.mean(dim=0)
                # cov_matrix = torch.cov(adv_debais.T)
                # inv_cov_matrix = torch.inverse(cov_matrix)
                #
                # mahalanobis_distances = torch.tensor(
                #     [self.mahalanobis_distance(adv_debais[i], mean_vector, inv_cov_matrix) for i in range(adv_debais.size(0))])
                #
                # mahalanobis_loss = mahalanobis_distances.mean()

                #
                # loss = F.cross_entropy(adv_ori, target_y) - mse_loss

                # - F.cross_entropy(adv_debais, target_y)  # - 0.8 * term
                # loss = term  # F.cross_entropy(adv_ori, target_y) - F.cross_entropy(adv_debais, target_y) + term
                # loss = F.cross_entropy(chat_adv, target_y) + F.cross_entropy(adv_ori, target_y) - F.cross_entropy(adv_debais, target_y)
                # loss = F.cross_entropy(chat_adv, target_y) - F.cross_entropy(adv_debais, target_y) + term2
                if mode is None:
                    ...
                    loss_kl = F.kl_div(F.softmax(adv_debais, dim=1).log(), F.softmax(ori_logit_debias, dim=1),
                                       reduction='batchmean')
                    loss = F.cross_entropy(adv_ori, target_y) - 3 * loss_kl
                elif mode == 'bafa_kl2':
                    loss_kl = F.kl_div(F.softmax(adv_debais, dim=1).log(), F.softmax(ori_logit_debias, dim=1),
                                       reduction='batchmean')
                    loss = F.cross_entropy(chat_adv, target_y) - 3 * loss_kl
                elif mode == 'bafa_kl3':
                    loss_kl = F.kl_div(F.softmax(chat_adv, dim=1).log(), F.softmax(chat_ori, dim=1),
                                       reduction='batchmean')
                    mse_loss = 0
                    feat_num = adv_pred.size(1)
                    for i in range(feat_num):
                        mse_loss += F.mse_loss(adv_pred[:, i], ori_pred[:, i], reduction="none").mean()
                    loss = F.cross_entropy(adv_debais, target_y) + loss_kl - mse_loss
                    ...
                    for i in range(feat_num):
                        mse_loss += F.mse_loss(adv_pred[:, i], ori_pred[:, i], reduction="none").mean()
                    loss = F.cross_entropy(adv_debais, target_y) - mse_loss
                    ...
                    # kl_debias
                    kl_loss = F.kl_div(F.softmax(adv_debais, dim=1).log(), F.softmax(ori_logit_debias, dim=1),
                                       reduction='batchmean')
                    # Combined loss
                    loss = 0.4 * ce_loss - 0.6 * kl_loss - mse_loss
                elif mode == 'bafa_kd2':
                    ce_loss = F.cross_entropy(adv_ori, target_y)
                    # kl_debias
                    kl_loss = F.kl_div(F.softmax(adv_debais, dim=1).log(), F.softmax(ori_logit_debias, dim=1),
                    ...
                    for i in range(feat_num):
                        mse_loss += F.mse_loss(adv_pred[:, i], ori_pred[:, i], reduction="none").mean()
                    loss = F.cross_entropy(adv_debais, target_y) - mse_loss
                elif mode == 'bafa_reverse':
                    loss = F.cross_entropy(adv_debais, target_y) - F.cross_entropy(adv_ori, target_y)
                elif mode == 'ori':
