...
class PGD():
    ...
    def perturb_bafa(self, x, query_embeddings, debais_well_q, target_y=None, model=None, y_best_group=None,mode=None, **kwargs):
        bound, step, iters, device = self.bound, self.step, self.iter, self.device
        if mode is None:
            mode = self.mode
        model.zero_grad()
        ...
        for i in range(iters):  # TODO argmax need to debias Y. So before argmax please use P0 Sigma-1
            ...
            if self.no_label:
                ...
                if mode  is None:
                    ...
                    loss_kl = F.kl_div(F.softmax(adv_debais, dim=1).log(), F.softmax(ori_logit_debias, dim=1), reduction='batchmean')
                    ...
                    for i in range(feat_num):
                        mse_loss += F.mse_loss(adv_pred[:, i], ori_pred[:, i], reduction="none").mean()
                    loss = F.cross_entropy(adv_debais, target_y) - 10 *mse_loss
                elif mode == 'ori':
