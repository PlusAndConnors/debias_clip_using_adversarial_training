...
def summarize_acc(correct_by_groups, total_by_groups,
                  stdout=True, return_groups=False, return_adj=False):
    all_correct, adj_correct = 0, 0
    all_total, adj_total = 0, 0
    min_acc = 101.
    ...
    for yix, y_group in enumerate(correct_by_groups):
        for aix, a_group in enumerate(y_group):
            ...
            all_correct += a_group
            accs_by_group[yix][aix] = acc
            all_total += total_by_groups[yix][aix]

    avg_acc = all_correct / all_total * 100
    adj_avg_acc = (accs_by_group[0][0] * 3498 + accs_by_group[0][1] * 184 + accs_by_group[1][0] * 56 + accs_by_group[1][
        1] * 1057) / (3498 + 184 + 56 + 1057)

    if stdout:
        average_str = f'Average acc: {int(all_correct):5d} / {int(all_total):5d} = {avg_acc:>7.3f}'
        adj_average_str = f'Adj Average acc: {adj_avg_acc:>7.3f}'
        robust_str = f'Robust  acc: {int(min_correct_total[0]):5d} / {int(min_correct_total[1]):5d} = {min_acc:>7.3f}'
        print('-' * len(average_str))
        print(average_str, '->', adj_average_str)
        print(robust_str)
        print('-' * len(average_str))
    if return_adj:
        return avg_acc, min_acc, groups_accs, adj_avg_acc
