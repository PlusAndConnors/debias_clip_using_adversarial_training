...
class Adversarial:
    def __init__(self, opts, modal):
        ...
        # K_x, K_y, K_s, K_z
        # 'RFFGaussian' CfgNode({'rff_dim': 3000, 'sigma_numel_max': 4000})
    ...
    def solver(self, X, Y, S, Z=None):
        ...
        # L2 Norm
        self.norm2_b_y = torch.linalg.norm(b_y, 2)
        self.norm2_b_s = torch.linalg.norm(b_s, 2)

        lamba_s = self.lambda_s / (1. - self.lambda_s)  # Image 0.7/0.3 , Text 0.1/0.9
        b = b_y / self.norm2_b_y - lamba_s * b_s / self.norm2_b_s

        if not Z is None:
            self.norm2_b_z = torch.linalg.norm(b_z, 2)
            lamba_z = self.lambda_z / (1. - self.lambda_z)
            b_z = lamba_z * b_z / self.norm2_b_z
            b += b_z

        b = (b + b.t()) / 2

        # Bu=λCu
        encoder = self.process2(X, Z, c, b, n)
    ...
    def process2(self, X, Z, c, b, n):
        ...
        # U (3000,1) self.kernel_x.w (3000,768)
    ...
    def r_(self, sorted):  # apply lambda
        #########################################
