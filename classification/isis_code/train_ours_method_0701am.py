...
from base_backup_0701am import AlternatingOptimizer, Use_original, Unlearn
...
def main():
    ...
    parser.add_argument("--config-file", default="configs/debias_waterbird_mode.yaml", metavar="FILE", # debias_waterbird_mode.yaml
    ...
    for i, (_, _, labels_train_y, labels_train_s, labels_train_y_gt) in enumerate(traindata):
        ...
        model.train_sud_loss(iter, True, args.att_mode) if cfg.mode == 'sud' else model.train(iter)
        # test
        model.test()
