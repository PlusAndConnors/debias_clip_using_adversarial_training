...
from base_backup_CAN import Use_original, Unlearn
...
def main():
    ...
    parser.add_argument('--att_mode', default=None, type=str)  # mse or bafa or ori
    ...
    for i, (_, _, labels_train_y, labels_train_s, labels_train_y_gt) in enumerate(traindata):
        ...
        if args.learn_mode == 'CAN' or args.learn_mode == 'BCAU':
            model.info_img(text_embeddings, labels_train_y, test_loader_base, testdata, cfg.mode)
        else:
            model.info(text_embeddings, labels_train_y, test_loader_base, testdata)
        # model.cifar_test2()

        if cfg.mode == 'CAN':
            pre_txt_feature = get_embeddings(text_descriptions, model.text_model.model, args).float().to(cfg.device)
            # print('new_text_embedding :', model.text_model.target_layer(pre_txt_feature.to(args.device)).value)
            # print('original_text_embedding : ', text_embeddings)
            model.train_txt_sud_loss(iter, True, args.att_mode, args.learn_mode, pre_txt_feature)
        elif cfg.mode == 'BCAU':
            pre_txt_feature = get_embeddings(text_descriptions, model.text_model.model, args).float().to(cfg.device)
            # Bias Corrective Augmentation for text
            # model.train_sud_loss_txt_tuning(iter, True, args.att_mode, args.learn_mode, pre_txt_feature)
            model.train_sud_loss_both_tuning(iter, True, args.att_mode, args.learn_mode, pre_txt_feature)
            # Bias Corrective Augmentation for image
            # Bias Corrective Update for text
            # Bias Corrective Update for image


        elif cfg.mode == 'sud':
            model.train_sud_loss(iter, True, args.att_mode, args.learn_mode)
        else:
            model.train(iter)
        # test
        model.test(False, args.learn_mode) if (cfg.mode =='CAN' or cfg.mode =='BCAU') else model.test()
        model.cifar_test2()
